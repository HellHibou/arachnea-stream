...
- Antenne Réunion (`antennereunion-fr.yaml`) : les tuiles `idType=7` de la page d'accueil pointent vers des routes applicatives (`/page/MenuSerie`, `/offers/`, …) que `listContent` ne peut pas résoudre (il attend un id Long), donc la home n'expose plus de catégories cliquables. Investiguer comment l'application officielle résout ces routes (endpoint dédié ou ids catalogue figés) pour réintroduire un bloc `categories`.
