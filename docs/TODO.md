...
- **Obscura : phases 4–6 de l'intégration embarquée** : le solveur HTTP Cloudflare (Phase 2 : `send` / `refresh_cloudflare` / `refresh_cloudflare_fresh` sur page éphémère stealth, protocole de clearance borné, cache `CachedChaserSession` partagé) et la session de page persistante `ObscuraPageSession` (Phase 3 : `navigate`/`fetch` in-page/`click_and_wait` avec protocole de challenge sur la même page/metadata/token Turnstile, page retenue sur thread dédié `!Send` pilotée par canal de commandes) sont implémentés et validés localement (Windows). Restent : les PoC réels sur cible autorisée (gates Phases 2 et 3, y compris l'équivalence du clic `element.click()` pour les widgets ciblés), l'adaptateur intercepteur `Fulfill` pour le proxy Arachnea in-process (Phase 4, refusé explicitement pour l'instant), la validation multiplateforme macOS/Linux/Windows et les mesures RSS/CPU/latence (Phase 5), puis la bascule documentaire et le retrait progressif de `chaser-cf` (Phase 6). Suivi détaillé : `docs/dev-tracking/obscura-embedded-engine-implementation-plan.md`.
