...
### Changed
- **Cloudflare browser fallback contract**: Browser solvers no longer fetch HTML after a rejected cookie handoff. After bounded refresh attempts, the client returns `CloudflareBlocked`; chaser-cf no longer supports persistent browser page sessions or direct HTML transport.
