...
import { isDashGeneratedAudioTrackId } from '@/composables/video/video-js-media-renderer/dash'
import {
 ...
 * Higher scores indicate better matches. Identifiers generated by the DASH source
 * handler are ignored so that language, kind and label keep driving the match.
 *
...
function getTrackMatchScore(
  ...
  if (preference.id && id === preference.id && !isDashGeneratedAudioTrackId(preference.id)) {
    return 100
  }
...
/**
 * Reapplies only the persisted audio and subtitle choices after a streaming
 * engine has replaced its track lists.
 *
 * @param player Video.js player whose track lists were renewed.
 * @param state Persisted player state containing the track preferences.
 */
export function restorePersistedTrackPreferences(
  player: VideoJsPlayer,
  state: VideoJsPlayerState | null,
) {
  if (!state) {
