...
use super::{JsonPersistenceFileCodec, PersistedRecord, PersistenceFileCodec, PersistenceStore};
...
/// Typed adapter over the deprecated namespace-based store contract.
///
/// This adapter is intentionally transitional: it keeps map conversion inside
/// the persistence layer so domains can migrate to repositories before the
/// SQLite backend is introduced.
pub struct LegacyTypedEntityStore<E: PersistentEntity> {
    config: PersistenceStoreConfig,
    store: Arc<dyn PersistenceStore>,
    marker: PhantomData<E>,
}

impl<E: PersistentEntity> LegacyTypedEntityStore<E> {
    /// Binds one entity schema to one named legacy store namespace.
    pub fn new(
        store: Arc<dyn PersistenceStore>,
        config: PersistenceStoreConfig,
    ) -> anyhow::Result<Self> {
    ...
    fn record(&self, entity: &E) -> anyhow::Result<PersistedRecord> {
        let document = encode(&self.config, entity)?;
        let fields = document_to_json(&document)?;
        let expires_at = self
            .config
            .schema
            .expiration_field()
            .and_then(|field| document.fields.get(field.name()))
            .map(|value| match value {
                StoredValue::DateTime(value) => from_nanos(*value),
                _ => anyhow::bail!("expiration field has an invalid value"),
            })
            .transpose()?;
        Ok(PersistedRecord {
            format_version: 1,
    ...
    fn entity(&self, record: PersistedRecord) -> anyhow::Result<Option<E>> {
        let Some(fields) = record.fields else {
            return Ok(None);
        };
        let document = document_from_json(&self.config.schema, fields)?;
        if expired(&self.config.schema, &document)? {
            return Ok(None);
        }
        Ok(Some(decode(&self.config.schema, &document)?))
    }
}

#[async_trait]
impl<E: PersistentEntity> TypedEntityStore<E> for LegacyTypedEntityStore<E> {
    async fn get(&self, key: &E::Key) -> anyhow::Result<Option<E>> {
        let transaction = self.store.transaction(&self.config.name).await?;
        let Some(record) = transaction.get(&key.storage_key()).await? else {
            return Ok(None);
        };
        self.entity(record)
    }
    ...
    async fn put(&self, entity: &E) -> anyhow::Result<()> {
        let transaction = self.store.transaction(&self.config.name).await?;
        transaction
            .put(entity.key().storage_key(), self.record(entity)?)
            .await?;
        transaction.commit().await
    }

    async fn put_all(&self, entities: &[E]) -> anyhow::Result<()> {
        let records: anyhow::Result<Vec<_>> = entities
            .iter()
            .map(|entity| Ok((entity.key().storage_key(), self.record(entity)?)))
            .collect();
        let transaction = self.store.transaction(&self.config.name).await?;
        for (key, record) in records? {
            transaction.put(key, record).await?;
        }
        transaction.commit().await
    }
    ...
    async fn delete(&self, key: &E::Key) -> anyhow::Result<()> {
        let transaction = self.store.transaction(&self.config.name).await?;
        transaction.delete(&key.storage_key()).await?;
        transaction.commit().await
    }

    async fn find(&self, query: &EntityQuery<E>) -> anyhow::Result<Vec<E>> {
        let transaction = self.store.transaction(&self.config.name).await?;
        let records = transaction.find_by_fields(&serde_json::Map::new()).await?;
        let mut entities = Vec::new();
        for (_, record) in records {
            if let Some(entity) = self.entity(record)? {
                let document = encode(&self.config, &entity)?;
                if matches(&self.config, &document, query)? {
...
fn document_to_json(document: &EntityDocument) -> anyhow::Result<serde_json::Map<String, Value>> {
    document
        .fields
        .iter()
        .map(|(name, value)| {
            serde_json::to_value(value)
                ...
                .map_err(|error| {
                    anyhow::anyhow!("failed to serialize entity field '{name}': {error}")
                })
        })
        .collect()
}
...
fn document_from_json(
    ...
    for (name, value) in fields {
        ...
        let value = match field.field_type {
            FieldType::String => StoredValue::String(serde_json::from_value(value)?),
            FieldType::Integer => StoredValue::Integer(serde_json::from_value(value)?),
            FieldType::Boolean => StoredValue::Boolean(serde_json::from_value(value)?),
            FieldType::DateTime => StoredValue::DateTime(serde_json::from_value(value)?),
            FieldType::Json => StoredValue::Json(value),
        };
        converted.insert(name, value);
    }
    let document = EntityDocument { fields: converted };
    validate_document(schema, &document)?;
    Ok(document)
}
