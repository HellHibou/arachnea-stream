...
The shell invokes `desktop_tabs_snapshot`, `desktop_tabs_open`, `desktop_tabs_activate`, and `desktop_tabs_close` (the latter two take an `id`) and listens for `desktop-tabs-changed` snapshots containing `tabs: [{ id, title }]` and `active`. These commands are restricted to the shell webview. Opening activates the new tab; closing the last tab creates a fresh home tab. Hidden tabs retain state and may continue media playback until closed. Tabs are not persisted across application restarts. Application capabilities must cover the host window and its child webviews.
