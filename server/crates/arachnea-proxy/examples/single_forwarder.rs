...
use arachnea_proxy::core::{ArachneaProxyCore, ProxyNode, UsageProfile};
use http::Uri;
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tower_service::Service;

// cargo run -p arachnea-proxy --example proxy_single_forwarder -- socks5://127.0.0.1:9050 example.com /
/// Runs an HTTP GET through one configured upstream proxy.
///
/// # Errors
///
/// Returns an error when the proxy URL is invalid, the proxy core cannot be
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let core = ArachneaProxyCore::new(
        UsageProfile::SingleForwarder(ProxyNode::from_url("forwarder", proxy_url)?).config(),
    )?;
